...
export async function getWindAnalytics(deviceId: number, range: string): Promise<WindAnalytics> {
  ...
  const speedReadings = await prisma.sensorReading.findMany({
  ...
  const dirReadings = await prisma.sensorReading.findMany({
...
export async function getRainAnalytics(deviceId: number): Promise<RainAnalytics> {
  ...
  const [todayReadings, weekReadings, monthReadings] = await Promise.all([
